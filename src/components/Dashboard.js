...
const Dashboard = ({ customers = [], onUpdateCustomer, onDeleteCustomer, onRefresh, onAddCustomer, imageDisplaySize = 'medium', onImageSizeChange }) => {
  ...
  const handleApplyInterestToAll = async () => {
    try {
      ...
      onRefresh();
      alert('Interest applied to all eligible customers successfully!');
    } catch (err) {
  ...
  const handleAddHistoricalCustomer = async (customerData) => {
    try {
      ...
      alert(`Error adding historical customer: ${err.message}`);
    }
  };
    ...
    <div className="dashboard">
      <div className="dashboard-header">
        ...
        <div className="dashboard-controls">
          ...
          <button onClick={() => setShowHistoricalForm(true)} className="btn-info">
            Add Historical Entry
          </button>
          <button onClick={handleApplyInterestToAll} className="btn-primary">
      ...
      <div className="customers-grid">
        {filteredCustomers.map(customer => {
          ...
          // Handle both possible property names for current balance
          ...
          return (
            <div key={customer.id} className="customer-card">
              <div className="customer-info">
                ...
                {goldWeight > 0 && (
                  <p><strong>Gold Weight:</strong> {goldWeight.toFixed(3)}g</p>
                )}
                {goldRate > 0 && (
                  <p><strong>Gold Rate:</strong> ₹{goldRate.toLocaleString()}/g</p>
                )}
                ...
                {/* MODIFIED: Changed currentBalance to lentAmount to calculate simple interest projection. */}
                {/* <p><strong>Next Month Interest:</strong> ₹{calculateMonthlyInterest(lentAmount).toLocaleString()}</p> */}
 ...
 <span className="interest-preview"> &emsp;&emsp;&emsp;Interest:₹{interestEarned.toLocaleString()}</span>
      ...
      {showHistoricalForm && (
        <HistoricalEntryForm
          onAddCustomer={handleAddHistoricalCustomer}
